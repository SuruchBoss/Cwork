...
describe('buildOtpauthUri', () => {
  it('produces a URI an authenticator app can scan', () => {
    const uri = buildOtpauthUri({
      ...
      accountName: 'hr.manager@cwork.example',
      issuer: 'Cwork',
    });

    expect(uri.startsWith('otpauth://totp/')).toBe(true);
    expect(uri).toContain('secret=JBSWY3DPEHPK3PXP');
    expect(uri).toContain('issuer=Cwork');
  ...
  it('escapes a label that contains a colon or a space', () => {
    ...
    expect(decodeURIComponent(label)).toBe('บริษัท ซีเวิร์ค จำกัด:someone@example.com');
  });
});
