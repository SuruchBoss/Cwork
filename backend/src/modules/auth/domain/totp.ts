 ...
 * second factor.
 *
 * No I/O, no clock of its own — the caller passes the time. See
 * [ADR-0002](../../../../docs/adr/0002-pure-domain-logic.md).
 */
...
export interface OtpauthUriInput {
  ...
  accountName: string;
  /** The organisation, so someone with several accounts can tell them apart. */
  issuer: string;
  digits?: number;
  stepSeconds?: number;
}
